...
public class HostInterface
{
	...
	{
		if (ifaddr == null || ifaddr.length() == 0) {
			Debug.message("[Error] setInterface invalid ifaddr [" + ifaddr + "]");
			return;
		}

		if (HostInterface.isIPv4Address(ifaddr) == false && HostInterface.isIPv6Address(ifaddr) == false)
		{
			Debug.message("[Error] setInterface invalid ifaddr [" + ifaddr + "]");
			return;
		}

		Debug.message("[OK] setInterface ifaddr [" + ifaddr + "]");
		ifAddress = ifaddr;
	}
