		...
		try
		{
			...
			while (0 < contentLen)
			{
				...
				while (readCnt < contentLen)
				{
					...
					// 为优酷APP做的workaround,因为它发来的content length可能大于实际发来的数据长度
					if (readCnt > 2000 && contentLen > readCnt)
					{
						if (contentBuf.toString().contains("pl.youku.com")
								&& contentBuf.toString().contains("SetAVTransportURI"))
						{
		...
		if (in != null)
		{
			try
			{
				...
				if (dataSplit.length == 1)
				{
					...
					setIsSingleSend(true);//单字节发送
					contentByte[0] = (byte) (line.getBytes()[0]);
					setContent(contentByte);//设置内容
					return true;
				...
				contentByte[0] = (byte) (dataSplit[2].getBytes()[0]);
				short type = contentByte[0];
				if (type == 0x11)
				{
				...
				Debug.message("Cur=" + System.currentTimeMillis() + "      Snd=" + dataSplit[1]);
	...
	public boolean readQuickly(DatagramSocket serverSock)
	{
		...
		if (serverSock != null)
		{
			...
			try
			{
				...
				String dataStr = line;
				String[] dataSplit = dataStr.split("#");//分割数据
				if (dataSplit.length != 3)//说明报文不完整，丢包
				{
					// UDP通道不支持单字节命令
