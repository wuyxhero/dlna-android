...
public abstract class NanoHTTPD
{
	...
	protected ContentDirectory mCDS = null;

	protected MediaServer mMediaServer = null;
	...
	public static class Response
	{
		...
		public enum Status
		{
			OK(200, "OK"), CREATED(201, "Created"), ACCEPTED(202, "Accepted"), NO_CONTENT(204, "No Content"), PARTIAL_CONTENT(
					...
					"Not Found"), RANGE_NOT_SATISFIABLE(416, "Requested Range Not Satisfiable"), INTERNAL_ERROR(500,
	...
	public static interface SessionStateListener
	{
		...
		public void sesstionState(int state);
	}
	...
	protected class HTTPSession implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (r == null)
				{
					Response.error(outputStream, Response.Status.INTERNAL_ERROR,
				...
				} else
				{
					...
					{
						String resId = parms.get(ContentDirectory.CONTENT_ID);
						conId = mMediaServer.getContentDirectory().contentServiceRequestReceived(resId);
					}

					r.setRequestMethod(method);
					r.send(outputStream);

					if (mMediaServer != null && conId != -1)
					{
						mMediaServer.getContentDirectory().contentServiceRequestFinished(conId);
					}
				}
		...
		private void decodeHeader(BufferedReader in, Map<String, String> pre, Map<String, String> parms,
				Map<String, String> header) throws InterruptedException
		{
			try
			{
				...
				if (st.hasMoreTokens())
				{
					...
					while (line != null && line.trim().length() > 0)
					{
						int p = line.indexOf(':');
						if (p >= 0)
							header.put(line.substring(0, p).trim().toLowerCase(Locale.getDefault()), line.substring(p + 1)
									.trim());
						line = in.readLine();
					}
				}
			...
			try
			{
				...
				while (mpline != null)
				{
					...
					while (mpline != null && mpline.trim().length() > 0)
					{
						...
						if (p != -1)
						{
							item.put(mpline.substring(0, p).trim().toLowerCase(Locale.getDefault()), mpline.substring(p + 1)
									.trim());
						}
						mpline = in.readLine();
					}
					if (mpline != null)
					{
						...
						while (st.hasMoreTokens())
						{
							...
							if (p != -1)
							{
								disposition.put(token.substring(0, p).trim().toLowerCase(Locale.getDefault()),
