...
public class HTTPUSocket
{
	...
	public boolean open(String bindAddr, int bindPort)
	{
		...
		try
		{
			...
			ssdpUniSock = new DatagramSocket(null); // 指定Null很重要，否则Java会自动随机选个可用端口来绑定
			ssdpUniSock.setReuseAddress(true); // 绑定之前先设置Reuse
			ssdpUniSock.bind(bindInetAddr); // 然后再绑定
			System.out.println("HTTPUSocket REUSEADDR is enabled: " + ssdpUniSock.getReuseAddress()); // 返回结果是true，说明才有效
		} catch (Exception e)
		{
			Debug.warning(e);
			return false;
	...
	public boolean post(String addr, int port, String msg)
	{
		...
		try
		{
			//Debug.message("postMessageTo: " + addr + ":" + port + " msg = " + msg);
			//Debug.message("postMessageTo: " + addr + ":" + port);
			InetAddress inetAddr = InetAddress.getByName(addr);
			byte[] ms = msg.getBytes();
			DatagramPacket dgmPacket = new DatagramPacket(msg.getBytes(), ms.length, inetAddr, port);
			if (ssdpUniSock != null && dgmPacket != null)
			{
	...
	// private byte ssdvRecvBuf[] = new byte[SSDP.RECV_MESSAGE_BUFSIZE];
