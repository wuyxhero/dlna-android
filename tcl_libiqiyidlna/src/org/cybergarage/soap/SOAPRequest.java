...
public class SOAPRequest extends HTTPRequest
{
	...
	public SOAPRequest()
	{
		...
		setMethod(HTTP.POST);
		setStringHeader("User-Agent", "IQIYIDLNA/iqiyidlna/1.0");
	}
	...
	////////////////////////////////////////////////
	/*
	 * 这里增加是否需要等待SERVER端的回复
	 */
	public SOAPResponse postMessage(String host, int port, boolean isNeedReply, boolean isKeepAlive)
	{
		if (isNeedReply == false)//不需要回复
		{
			if (justPost(host, port, isKeepAlive) == false)
			{
				return null;//发送失败
			}
			return new SOAPResponse();//发送成功
		}

		HTTPResponse httpRes = post(host, port, isKeepAlive);//是否支持长连接
	...
	public void print()
	{
		...
		Debug.message(toString().replace(HTTP.CRLF, "	"));
		if (hasContent() == true)
		{
		...
		Debug.message(rootElem.toString().replace(HTTP.CRLF, "	"));
		System.out.println("-------------------------------DUMP SOAPRequest [End]-------------------------------");
	}
}
